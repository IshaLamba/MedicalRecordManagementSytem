...
public class patdelete extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			ResultSet rs;
			int i=st.executeUpdate("delete from patient where email='"+Email+"'");
			System.out.println("Query executed");
			if(i>0)
			{
				out.println("Record Succesfully Deleted.....\n\n\n");
				RequestDispatcher rd=request.getRequestDispatcher("/aplist.jsp");
				rd.include(request,response);
			}
			else
			{
				out.println("Deletion Failed, Please try again.");
				RequestDispatcher rd=request.getRequestDispatcher("/pdelete.jsp");
				rd.include(request,response);
			}
